...
function polygonPoints(outerRadius, innerRadius, number, lw) {
  var radAngle = outerRadius > innerRadius ? Math.PI / 2 : -Math.PI / 2;
  var radAlpha = Math.PI / number;
  var points = [];

  for (var i = 1; i <= number * 2; i++) {
    var r = i % 2 ? outerRadius : innerRadius;
    var alpha = radAlpha * i + radAngle;
    var cos = Math.cos(alpha);
    var sin = Math.sin(alpha);
    var x = r * cos - lw / 2 * cos;
    var y = r * sin - lw / 1.75 * sin;
